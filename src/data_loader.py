...
def load_all_fasta_files(data_dir, file_pattern=None):
    ...
    for filename in sorted(os.listdir(data_dir)):
        if not (filename.endswith('.fasta') or filename.endswith('.fa') or filename.endswith('.fas')):
            continue
...
def prepare_data(df, label_to_idx, max_length=None, encoding='onehot', 
                 test_size=0.15, val_size=0.15, random_state=42,
                 batch_size=32, strip_alignment_gaps=True):
    """
    ...
    if strip_alignment_gaps:
        ...
        print(f"Gap-stripped sequences: length range {min(lengths)}–{max(lengths)} bp "
...
def create_dataloaders(X_train, y_train, X_val, y_val, X_test, y_test,
                       max_length=None, encoding='onehot', batch_size=32,
    ...
    if strip_alignment_gaps:
        ...
        print(f"Gap-stripped: train lengths {min(len(s) for s in X_train)}–"
    ...
    print(f'DataLoaders ready: train={len(train_loader)}, '
