...
def evaluate_model(model, test_loader, idx_to_label, device='cpu',
                   save_dir='reports/figures', model_name='model'):
    ...
    return {'accuracy': accuracy, 'macro_f1': f1, 'macro_precision': precision,
            'macro_recall': recall, 'y_true': y_true, 'y_pred': y_pred,
            'y_proba': y_proba, 'report': report}
...
def plot_confusion_matrix(y_true, y_pred, class_names, save_dir, model_name):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/{model_name}_confusion_matrix.png', dpi=150, bbox_inches='tight')
    plt.show()
...
def plot_roc_curves(y_true, y_proba, class_names, save_dir, model_name):
    ...
    ax.set_title(f'{model_name} — ROC Curves'); ax.legend(loc='lower right')
...
def plot_per_class_metrics(y_true, y_pred, class_names, save_dir, model_name):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/{model_name}_per_class_metrics.png', dpi=150, bbox_inches='tight')
    plt.show()
