...
def sequence_stats(df, save_dir=None):
    ...
    print(f"Sequence length: {df['seq_length'].min()} - {df['seq_length'].max()} "
...
def _plot_distributions(df, save_dir):
    ...
    axes[1].hist(df['seq_length'], bins=50, color='steelblue', edgecolor='white', alpha=0.8)
    axes[1].set_title('Sequence Length Distribution (pol)', fontsize=13)
    axes[1].set_xlabel('Length (bp)')
    axes[1].set_ylabel('Count')
    axes[1].axvline(df['seq_length'].median(), color='red', linestyle='--',
    ...
    plt.suptitle('HIV-1 pol Gene Dataset — Exploratory Data Analysis', fontsize=15, y=1.02)
    plt.tight_layout()
